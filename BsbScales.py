    ...
    def getKapsSortedByScale(self, addExt = ""):
        ...
        for key in keyList:
            for scale in self.scales[key]:
                fileName, _ = splitext(scale)
                fileName = fileName.split("/")[-1]
                lst.append(fileName+addExt)
        return lst
            
               
if __name__== "__main__":
    directory = "C:/Users/will/Desktop/SWETEST"
    bsbScales = BsbScales(directory)
    #bsbScales.printScales()
    for ea in bsbScales.getKapsSortedByScale(".zxy"):
        print ea
